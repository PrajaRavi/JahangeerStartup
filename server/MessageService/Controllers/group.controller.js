  ...
  try {
    ...
    */
    ...
    /*
      Extract body data
    */
    const { groupName, groupDescription, members, groupSettings } = req.body;
    /*
      Validate group name
    */
    if (!groupName?.trim()) {
    ...
    */
    ...
    const validMembers = JSON.parse(members).map(
    ...
    */
    ...
    if (groupSettings && Array.isArray(JSON.parse(groupSettings))) {
      validatedSettings = JSON.parse(groupSettings).filter((setting) =>
    ...
    */
    ...
    const newGroup = await Group.create({
      ...
      groupSettings: validatedSettings,
      members: NewMembers,
...
export const GetallGroup = async (req, resp) => {
  try {
    ...
    let data = await Group.find({
      $or: [{ admins: userId }, { "members.userID": userId }],
    });
    if (data.length > 0) {
...
export const UpdateGroupWithID = async (req, resp) => {
  try {
    let { userId, groupName, groupDescription, members, groupSettings } =
      req.body;
    ...
    let data = await Group.updateOne(
      { _id: userId },
      {
        $set: {
          ...
          members,
          groupSettings,
          groupProfileImage: req.file.filename || "Unknown file",
        },
      },
    );
    ...
    if (data) {
      return resp
        .status(200)
        .send({ success: true, msg: "updated bhaiya!!!!!" });
    }
    console.log(data);
  } catch (error) {
    console.log(error);
    return resp
      .status(500)
      .send({ success: false, msg: "error in Updategroup" });
  }
};
...
export const GetLastMsgOFGroup = async (req, resp) => {
  try {
    if (!req.query.GroupID)
      return resp.status(200).send({ success: false, msg: "Nothing bhai!!!!" });
    let data = await MsgModel.find({ GroupID: req.query.GroupID }).sort({
      createdAt: -1,
    });
...
export const UpdateGroupMembers = async (req, resp) => {
  try {
    ...
    let data = await Group.updateOne(
      ...
      { $set: { "members.$.LastMsgID": msgid, LastMsgID: msgid } },
    );
    if (data) {
      return resp
        .status(200)
        .send({ success: true, msg: "successfully updated!!!" });
    }
    return resp
      .status(200)
      .send({ success: false, msg: "Kuch to hua hai bhai!!!" });
  } catch (error) {
    console.log(error);
    return resp
      .status(500)
...
return
*/
    if (!userID || GroupIDArray?.length == 0)
      return resp.send({ success: false, msg: "kuch aya hi nahi hai!!!!" });
      ...
      GroupIDArray.map(async (GroupID) => {
        ...
        if (data.length > 0) {
          ...
          if (lastmsgId) {
            ...
            let groupmessages = await MsgModel.find({
              GroupID: GroupID,
              _id: { $gt: lastmsgId },
            });
